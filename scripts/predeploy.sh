...
print_step "Executando formata.py (black + isort)..."
...
CRITICAL_DEPS=("Django" "streamlit" "psycopg2-binary" "pandas")
...
if "$PYTHON_BIN" manage.py makemigrations --dry-run --check >/dev/null 2>&1; then
    ...
    print_warning "makemigrations detectaria mudanças de model — lembrar que o SGD NÃO deve gerar migrações (modelos já existem no banco)"
fi
...
if [ $ERRORS -eq 0 ]; then
    ...
    echo -e "  1. Anotar em documentacao/Historico.md"
    echo -e "  2. git commit"
